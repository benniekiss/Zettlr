...
import { linter, type Diagnostic, type Action, setDiagnosticsEffect } from '@codemirror/lint'
...
import { changesFieldEffectFactory, getDiagnostics, hideLinterToolTipEffect, type LintRange, prepareDiagnostics, refreshLinterEffect, TEXTNODE_FILTER } from './utils'
...
function cancelLTProcess (view: EditorView, ranges: LintRange[]): boolean {
  if (view.state.field(languageToolState).cancel === true) {
    console.log('[Language Tool] Cancelling request...')

    view.dispatch({
      effects: [
        updateLTState.of({ running: false, cancel: false }),
      ]
    })
  ...
  view.dispatch({
    effects: [
      // Add any yet-to-be linted chunks, including the current one
      ltChangesEffect.of(ranges)
    ]
  })
...
const ltLinter = linter(async view => {
  ...
  // of processing several chunks from a previous dispatch. We need
  // to send an cancel message so that the running process can
  // cleanup and terminate early.
  if (view.state.field(languageToolState).running) {
    return getDiagnostics(view.state, 'language-tool')
  }

  const { languageToolCharsPerRequest } = view.state.field(configField)
  ...
  // This has to run after `prepareDiagnostics` because it resets the accumulated changes
  view.dispatch({
    effects: [
      updateLTState.of({ running: true }),
  ...
  for (let range = 0; range < ranges.length; range++) {
    ...
    for (let chunk = 0; chunk < chunks.length; chunk++) {
      ...
      const remainingRanges = ranges.slice(range + 1).concat(chunks.slice(chunk).map(c => ({ from: c.start, to: c.end })) as LintRange[])
      if (cancelLTProcess(view, remainingRanges)) {
        return diagnostics
      }
      ...
      // At this point, we have only valid suggestions
      // that we can now insert into the document.
      diagnostics.push(...generateLTDiagnostics(view.state, ltSuggestions.matches, textNodes, start))

      // Return the diagnostics as soon as possible
      view.dispatch({
        effects: setDiagnosticsEffect.of(getDiagnostics(view.state, 'language-tool', false).concat(diagnostics))
      })
    }
  }
