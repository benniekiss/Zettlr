...
export default class AssetsProvider extends ProviderContract {
  ...
  async setSnippet (name: string, content: string, dir?: string): Promise<boolean> {
    ...
    try {
      ...
      if (dir === undefined) {
        this._logger.error(`[Assets Provider] Could not save snippet file: ${String(err.message)}`, err)
      } else {
  ...
  async removeSnippet (name: string, dir?: string): Promise<boolean> {
    ...
    try {
      ...
      if (dir === undefined) {
        this._logger.error(`[Assets Provider] Could not remove snippet file: ${String(err.message)}`, err)
      } else {
  ...
  async renameSnippet (name: string, newName: string, dir?: string): Promise<boolean> {
    ...
    try {
      ...
      if (dir === undefined) {
        this._logger.error(`[Assets Provider] Could not rename snippet file: ${String(err.message)}`, err)
      } else {
