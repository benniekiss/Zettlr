...
watch(format, function (value) {
  ...
  if (globalProfile !== undefined) {
    const prof = profileMetadata.value.find(prof => prof.name === globalProfile)

    if (prof !== undefined) {
      outputFilename.value = prof.outputFile
      outputFilenameIsAbsolute.value = isAbsolutePath(prof.outputFile)
  ...
  if (wsProfile !== undefined) {
    const prof = workspaceProfileMetadata.value.find(prof => prof.name === wsProfile)
  ...
  if (customProfile !== undefined) {
    ...
    outputFilenameIsAbsolute.value = false
  }
