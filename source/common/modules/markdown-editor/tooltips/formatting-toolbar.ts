...
import { applyBold, applyCode, applyComment, applyItalic, insertBracketedSpan, insertImage, insertLink } from '../commands/markdown'
...
function getToolbar (state: EditorState): Tooltip[] {
  ...
  return [{
    ...
    create: (view) => {
      ...
      underline.onmousedown = function (event) { insertBracketedSpan(view, '.underline') }
      highlight.onmousedown = function (event) { insertBracketedSpan(view, '.mark') }
